...
def trainNN(challenges, responses):
    ...
    with open("models/NN/8/params.txt", "w") as file:
...
def testNN(challenges, responses, model):
    ...
    with open("models/NN/8/params.txt", "a") as file:
    ...
    plt.legend()
    model_name = f"NN"
    plt.savefig(f'models/NN/8/{model_name.lower().replace(" ", "_")}_confidence_distribution.png')
    print(f"Confidence curves saved to models/NN/8/{model_name.lower().replace(' ', '_')}_confidence_distribution.png")
...
def plot_train(model_name, train_losses, val_losses, val_accuracies):
    ...
    plt.tight_layout()
    plt.savefig(f'models/NN/8/{model_name.lower().replace(" ", "_")}_training_curves.png')
    print(f"Training curves saved to models/NN/8/{model_name.lower().replace(' ', '_')}_training_curves.png")
...
if __name__ == "__main__":
    ...
    # Save the model
    torch.save(model.state_dict(), f"models/NN/8/nn_model.pth")
